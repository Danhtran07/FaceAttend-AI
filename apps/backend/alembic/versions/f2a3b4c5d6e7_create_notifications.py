...
def upgrade() -> None:
    ...
    if not inspector.has_table("notifications"):
        ...
        op.create_table(
            ...
            sa.UniqueConstraint("employee_id", "notification_type", "attendance_date", name="uq_notification_employee_type_date"),
        )

    indexes = inspector.get_indexes("notifications") if inspector.has_table("notifications") else []
    if not any(index["name"] == "ix_notifications_id" for index in indexes):
        op.create_index("ix_notifications_id", "notifications", ["id"])
    if not any(index["name"] == "ix_notifications_employee_id" for index in indexes):
...
def downgrade() -> None:
    ...
    if inspector.has_table("notifications"):
        ...
        if any(index["name"] == "ix_notifications_id" for index in indexes):
            op.drop_index("ix_notifications_id", table_name="notifications")
        if any(index["name"] == "ix_notifications_employee_id" for index in indexes):
